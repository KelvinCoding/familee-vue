    ...
    <v-layout justify-start column text-xs-center>
      <v-flex v-show="isMounted">
        <v-layout v-bind="layoutBinding">
          <v-flex v-for="(registry, index) in registries" :key="index" md4 xs12>
            <RegistryCard
              :title="registry.Title"
              :logo-component="registry.LogoComponent"
              :link="registry.Link"
            />
          </v-flex>
        </v-layout>
...
export default {
      ...
      registries: [
        {
          ...
          Link:
            'https://www.amazon.com/wedding/johanna-klay-kelvin-rodriguez-fort-worth-june-2019/registry/1QP19G9Z3N4H9'
        },
        {
          Title: 'Target',
          LogoComponent: TargetLogo,
          Link:
            'https://www.target.com/gift-registry/giftgiver?registryId=6dd3156f4d4b437198dda6410afdd0cb&type=WEDDING'
        }
      ]
    }
  },
  computed: {
    layoutBinding() {
      const isBig = this.isMounted ? this.$vuetify.breakpoint.mdAndUp : true
